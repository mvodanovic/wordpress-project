...
openssl req -newkey rsa:4096 --passout pass:"${CA_PASSPHRASE}" -keyform PEM -keyout "${CA_NAME}.ca.key" \
  -subj "/O=${CA_ORG_NAME}/CN=${CA_COMMON_NAME}" -new -x509 -days 3650 -outform PEM -out "${CA_NAME}.ca.crt"
...
openssl x509 -req -in "${DOMAIN_NAME}.client.req" --passin pass:"${CA_PASSPHRASE}" -CA "${CA_NAME}.ca.crt"\
